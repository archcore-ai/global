...
1. `research` is added as a knowledge type; `rnd` stays as decision-bound investigation. `evidence` is added as a knowledge type for one external material. Vocabulary in `concepts/research-and-evidence-types`.
...
4. The `research` expert path of `/archcore:plan` resolves to the `research` type; `rnd` is invoked by its name. Announced as a breaking change.
...
### Stage 3 — shared context, after the engine release (global, 4–6 h)

- [ ] `concepts/core-concepts` — 21 types, 7 relations, knowledge formula "how the system and its world work", the two types in the knowledge listing.
...
- [ ] `concepts/glossary`, `concepts/document-tracks`, `concepts/gated-tracks`, `concepts/naming-and-layout`, `architecture/system-map`, `market/moat-parity-and-gaps` — every "19 types" and "four relation types" updated.
...
3. Stage 3: no document in this repository states 19 types or four relations after the update.
...
- Stage 3 and stage 4 follow the stage 1 release, as the `rnd` RFC's adoption did.
