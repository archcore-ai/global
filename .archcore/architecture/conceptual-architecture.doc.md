...
**3. Runtime — guides the work (optional).** A higher-level experience over the engine: a small command surface and the gated tracks beneath it (see `concepts/gated-tracks`), which interview the user only where evidence is missing and produce linked typed documents. A gate fills a document and checks its shape; the method behind the content — how a decision is reached, how an investigation is run — stays with the host and the user (`architecture/store-not-method-engine`). It makes the engine effortless to use but is never required: any MCP-aware agent can use the engine directly, and every guardrail reaches that agent through the engine's hooks.
